...
# %%
with open("../data/snr-analysis-aizawa-50/aizawa_model_snr_50.dill", "rb") as f:
    model_n = dill.load(f)
...
# %%
with open("../data/snr-analysis-aizawa-50/aizawa_model_snr_60.dill", "rb") as f:
    model_n = dill.load(f)
...
ax2.scatter(
    ...
    color="#B03C2B",
    label="k ≥ 0.7",
    s=24,  # slightly larger than the blue points
    marker="^",  # triangle marker
    alpha=0.9,  # more visible
)
...
ax2.legend(markerscale=2.0)  # increase marker size in legend
...
fig.savefig(
    os.path.join(
        ...
        "results-diagnostics",
        "figures",
        "aizawa_snr_50_50_60_loo_plot.pdf",
    ),
    dpi=600,
)
