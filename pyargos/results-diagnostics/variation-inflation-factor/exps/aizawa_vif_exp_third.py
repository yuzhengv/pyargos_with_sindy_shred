...
for n_obs in n_obs_list:
    x_t = ags.generate_noisy_dynamical_systems(
        ...
        n=10**n_obs,
        dt=0.01,
        init_conditions=initial_value_df.iloc[16].values,
        snr=49,
    )
    ...
    with open(
        f"../../../data/n-analysis-aizawa-16/{system_name}_model_n_10_{int(n_obs * 10)}.dill",
        "wb",
    ) as f:
        dill.dump(model, f)
...
for snr_value in snr_list:
    ...
    # Save model object to file
    with open(
        f"../../../data/snr-analysis-aizawa-16/{system_name}_model_snr_{int(snr_value)}.dill",
        "wb",
    ) as f:
        dill.dump(model, f)
