...
model_ba = BayesianArgos(design_matrix=design_matrix, accelerator=False)
model_ba.run(
    mode="straight",
    parallel="yes",
    ncpus=None,
...
# Save the plot
plot_file_name = f"comparison_plot_ba_{iteration}.png"
...
overall_mse = np.mean(
    sum((gru_data[i] - sim_data[i]) ** 2 for i in range(latent_dim))
    / latent_dim
)
...
latent_mse_train_dict["Overall_MSE"] = overall_mse
...
# overall_mse = np.mean(latent_mse_test_values)
overall_mse = np.mean(
    sum(
        (gru_outs_test_np[:, i] - x_sim_test[:, i]) ** 2
        for i in range(latent_dim)
    )
    / latent_dim
)
...
latent_mse_test_dict["Overall_MSE"] = overall_mse
...
time_ranges = [(0, 100), (100, 200), (200, 275), (0, 275)]
results = {}
...
for start, end in time_ranges:
    ...
    rmse = np.sqrt(mse)
    # nrmse = rmse / np.linalg.norm(current_truth)

    # Calculate normalized RMSE
    normalised_rmse = np.linalg.norm(
        current_forecaster - current_truth
    ) / np.linalg.norm(current_truth)

    # Store results
    results[f"MSE_{range_label}"] = mse
    results[f"RMSE_{range_label}"] = rmse
    results[f"RMSE_normalized_{range_label}"] = normalised_rmse
    ...
    print(f"  Normalized RMSE: {normalised_rmse:.6f}")
